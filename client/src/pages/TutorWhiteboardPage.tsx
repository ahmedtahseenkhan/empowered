...
interface BoardFull extends BoardMeta { scene_data: Record<string, unknown>; }
...
const TutorWhiteboardPage: React.FC = () => {
    ...
    const saveBoard = async () => {
        if (!activeBoard || !storeRef.current) return;
        setSaving(true);
        try {
            ...
            setBoards((prev) =>
                prev.map((b) => b.id === activeBoard.id ? { ...b, title, updated_at: new Date().toISOString() } : b)
            );
        } finally {
            setSaving(false);
        }
    ...
    const handleMount = useCallback((store: ReturnType<typeof createTLStore>) => {
        storeRef.current = store;
        if (activeBoard?.scene_data && Object.keys(activeBoard.scene_data).length > 0) {
            try {
                loadSnapshot(store, activeBoard.scene_data as any);
            } catch {
                // empty board if snapshot is invalid
            }
        }
    }, [activeBoard]);
            ...
            <div className="flex h-[calc(100vh-8rem)] gap-4">
                ...
                <div className="w-60 flex-shrink-0 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-1">
                        {boards.length === 0 && (
                            <p className="text-xs text-gray-400 text-center mt-6 px-4">No boards yet. Click + to create one.</p>
                        )}
                        {boards.map((b) => (
