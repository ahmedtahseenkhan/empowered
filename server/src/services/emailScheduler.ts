...
const DEMO_REMINDER_HOURS = Number(process.env.DEMO_REMINDER_HOURS || 3); // same-day nudge
const DEMO_EARLY_REMINDER_HOURS = Number(process.env.DEMO_EARLY_REMINDER_HOURS || 24); // day-before reminder
...
async function queueDemoCallReminders(hoursAhead: number, reminderType: '24h' | '3h', idempotencyPrefix: string) {
    const now = new Date();
        ...
        try {
            await prisma.emailOutbox.create({
                data: {
                    ...
                    to_email: demo.email,
                    payload: { demoBookingId: demo.id, reminderType },
                    // Slot time is part of the key so a rescheduled demo gets a fresh reminder.
                    idempotency_key: `${idempotencyPrefix}:${demo.id}:${demo.slot_start_time.getTime()}`,
                },
            });
            console.log(`[EmailScheduler] Queued ${reminderType} demo call reminder for: ${demo.email}`);
        } catch (e: any) {
            if (e.code !== 'P2002') {
                console.error(`[EmailScheduler] Failed to queue ${reminderType} demo call reminder:`, e);
            }
        }
    }

    return upcomingDemos.length;
}

async function checkDemoCallReminders() {
    try {
        // Day-before reminder.
        const earlyCount = await queueDemoCallReminders(
            DEMO_EARLY_REMINDER_HOURS,
            '24h',
            'demo-call-reminder-24h-mentor'
        );

        // Same-day nudge. Keeps the original idempotency prefix so demos that already
        // received the legacy short-notice reminder are not emailed twice.
        const soonCount = await queueDemoCallReminders(
            DEMO_REMINDER_HOURS,
            '3h',
            'demo-call-reminder-mentor'
        );

        if (earlyCount + soonCount > 0) {
            console.log(`[EmailScheduler] Processed ${earlyCount} day-before + ${soonCount} same-day demo call reminders`);
        }
    } catch (error) {
...
export function startEmailScheduler() {
    ...
    console.log(`[EmailScheduler] Starting scheduler (interval=${SCHEDULER_INTERVAL_MS}ms, sessionReminder=${SESSION_REMINDER_HOURS}h, demoReminder=${DEMO_REMINDER_HOURS}h, demoEarlyReminder=${DEMO_EARLY_REMINDER_HOURS}h, paymentReminder=${PAYMENT_REMINDER_HOURS}h, earlyPaymentReminder=${PAYMENT_EARLY_REMINDER_HOURS}h)`);
